...
function gasRuntime() {
  const state={properties:new Map([['CRM_SHARED_SECRET',env.CRM_SHARED_SECRET],['SPREADSHEET_ID','test']]),cache:new Map(),tables:new Map(),networkFail:false,calls:0};
...
test('GAS saves once and repairs outbox after interrupted write',()=>{const {context,state}=gasRuntime();const first=sendGas(context);assert.ok(first.ok);state.tables.get('Уведомления').splice(1);assert.equal(sendGas(context).id,first.id);assert.equal(state.tables.get('Заявки').length,2);assert.equal(state.tables.get('Уведомления').length,2);});
