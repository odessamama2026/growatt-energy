...
export async function handleLead(request, env = process.env, send = fetch) {
  ...
  try {
    ...
    const response = await send(env.GOOGLE_CRM_WEBAPP_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(envelope),redirect:'follow',signal:AbortSignal.timeout(20000)});
